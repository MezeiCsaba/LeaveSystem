...
//	}
	...
	public Double getUserSumLeave(Long userId, int actYear) { // a tervezett és jóváhagyott szabik száma (az elutasított nem)
...
//	@PostConstruct
	public void init() throws ParseException {
		...
		Integer thisYear = Year.now().getValue();
		// -----------------------------------------------
		User user = new User("Mezei Csaba", "mezeicsaba72@gmail.com", passwordEncoder.encode("pass"), "USER", true);
		user.setStatus(true);
		userRepo.save(user);
		...
		user = new User("Admin", "admin@admin.com", passwordEncoder.encode("pass"), "ADMIN", true);
		user.setStatus(true);
		userRepo.save(user);

		leaves = new UserLeaves(user, thisYear, 20, 0, 0, 0);
		leaveService.saveLeave(leaves);
		...
		user = new User("Kiss Irén", "mezeicsaba72+kissiren@gmail.com", passwordEncoder.encode("pass"), "HR", true);
		user.setStatus(true);
		userRepo.save(user);
